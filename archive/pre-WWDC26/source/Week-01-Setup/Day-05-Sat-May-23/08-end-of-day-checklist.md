...
- [x] Why is MLX faster than PyTorch on a Mac? *(unified memory — no CPU↔GPU copy)*
- [x] What's the difference between Foundation Models and MLX? *(Apple's native shipped LLM vs. open-source framework you control)*
- [x] What does `terraform plan` do that `terraform apply` doesn't? *(plan shows the diff but mutates nothing; apply executes it)*
- [x] Where does Terraform store the "real cloud truth"? *(`terraform.tfstate`)*
- [x] What was the model you ran locally and roughly how big is it on disk? *(`mlx-community/gemma-3-1b-it-4bit`, ~600 MB at 4-bit quant)*

---

## ✅ Artifacts saved

- [x] Kickoff blog post is **live** with a public URL
- [x] Blog URL written to `~/Desktop/AI/Week-01-Setup/blog-posts.md`
- [x] Gemma 3 first-generation screenshot saved (for the next blog post)
- [x] `~/Desktop/AI/ml-tools/` exists with `mlx-env/` and `mlx-examples/`
